...
/* Forty-seven Acts come from the vocation tables; one more — Penance
   (Confession) — is described on its own and appears in no table. */
ok("acts from the vocation tables", acts.filter((a) => a.inVocationTables !== false).length, 47);
ok("and one described outside them",
   acts.filter((a) => a.inVocationTables === false).map((a) => a.name), ["Penance (Confession)"]);
ok("act count matches header", acts.filter((a) => a.inVocationTables !== false).length, faith.count);
...
ok("every row carries six marks",
   acts.filter((a) => a.inVocationTables !== false && a.marksFound !== 6).map((a) => a.name), []);

ok("every act has a name and a minimum",
   acts.filter((a) => !a.name?.trim() || !(a.pffMinimum > 0)).map((a) => a.name), []);

/* Every Act in the vocation tables is open to at least one vocation — that is
   what those tables are. Penance is described on its own page and named in no
   table, so it has no vocations to list. */
ok("every act is available to someone",
   acts.filter((a) => a.inVocationTables !== false && !a.vocations.length).map((a) => a.name), [])
