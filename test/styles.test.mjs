...
/* -- A prompt that scrolls ------------------------------------------------- */

/* The targeting prompt asks a great deal, and at full length it pushed its own
   Roll button off the screen. Its questions scroll and the button, in the
   dialog's footer, stays put — so the class must both cap the height and let
   the overflow scroll, or it does neither. */
const scroll = /\.cns5-prompt--scroll\s*\{([^}]*)\}/.exec(css)?.[1] ?? "";
ok("the long prompt caps its height", /max-height:/.test(scroll), true);
ok("and scrolls what does not fit", /overflow-y:\s*auto/.test(scroll), true);

const promptSource = await readFile(
  path.join(ROOT, "module", "helpers", "defence-prompt.mjs"),
  "utf8"
);
const targetingPrompt = promptSource.slice(promptSource.indexOf("export async function promptTargeting("));
ok(
  "the targeting prompt uses it",
  /class="cns5-prompt cns5-prompt--scroll"/.test(targetingPrompt.slice(0, 4000)),
  true
);
