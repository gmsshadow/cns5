...
/* Every button's action has somewhere to go. A data-action with no handler
   behind it renders as a perfectly good button that does nothing when pressed
   — indistinguishable on screen from one that works, and precisely the thing
   that cannot be seen without Foundry to click it in. */
const handlersIn = async (file) => {
  const code = await readFile(path.join(ROOT, "module", "sheets", file), "utf8");
  const block = /actions:\s*\{([\s\S]*?)\n\s{4}\}/.exec(code)?.[1] ?? "";
  return new Set([...block.matchAll(/^\s*(\w+):/gm)].map((m) => m[1]));
};

// Foundry supplies a few actions of its own to every sheet.
const builtIn = new Set(["editImage", "close", "toggleControls"]);

const actorActions = new Set([
  ...(await handlersIn("character-sheet.mjs")),
  ...(await handlersIn("npc-sheet.mjs")),
  ...builtIn
]);
const itemActions = new Set([
  ...(await handlersIn("item-sheet.mjs")),
  ...(await handlersIn("skill-sheet.mjs")),
  ...builtIn
]);

const unhandled = [];
for (const [dir, known] of [["actor", actorActions], ["item", itemActions]]) {
  for (const file of await walk(path.join(ROOT, "templates", dir), ".hbs")) {
    const text = await readFile(file, "utf8");
    for (const m of text.matchAll(/data-action="(\w+)"/g)) {
      if (!known.has(m[1])) unhandled.push(`${path.relative(ROOT, file)}: ${m[1]}`);
    }
  }
}
ok("every button's action has a handler", [...new Set(unhandled)], []);

/* -------------------------------------------- */

/* A spell cast from an item makes up to three rolls on one card: casting it out
   of the item, targeting it, and the target's save (p301). Without headings,
   two sets of dice appear and nothing says which was which. Every step the
   casting code names must be one the card can show, and the card must head its
   main dice when there is a step before them. */
const card = await readFile(path.join(ROOT, "templates", "chat", "check.hbs"), "utf8");
ok("the card shows the casting step", /\{\{#if castingStep\}\}/.test(card), true);
ok("with its own roll and target", /castingStep\.roll[\s\S]*castingStep\.need/.test(card), true);
ok("heads the targeting dice", /\{\{#if targetingHeading\}\}/.test(card), true);
ok("and heads the save", /\{\{#if saveHeading\}\}/.test(card), true);
ok(
  "the casting step comes before the dice",
  card.indexOf("castingStep") < card.indexOf("cns5-check__dice"),
  true
);

const actorCode = await readFile(path.join(ROOT, "module", "documents", "actor.mjs"), "utf8");
const stepKeys = [...actorCode.matchAll(/"(CNS5\.Step\.\w+)"/g)].map((m) => m[1]);
const langData = JSON.parse(await readFile(path.join(ROOT, "lang", "en.json"), "utf8"));
ok("every step named has words", [...new Set(stepKeys)].filter((k) => !langData[k]), []);
ok("and there are steps to name", new Set(stepKeys).size >= 7, true);
