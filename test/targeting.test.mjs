...
/* A Device is not a source a caster reads his own spell from: it casts with
   its maker's skill, from the Device itself, and its cost is worked out apart. */
ok("a device is not a casting source", ["deviceMage", "deviceOther"].filter((k) => k in CNS5.castingSources), []);
ok("from a device by a mage, a quarter", CNS5.deviceFatigue(7, true), 2);
ok("from a device by anyone else, half", CNS5.deviceFatigue(7, false), 4);
...
ok("and quartered too", CNS5.deviceFatigue(5, true), 2);
...
/* -- The bounds on a spell's Magick Resistance (p294) ---------------------- */

/* "The minimum MR of a spell is always 1 and the maximum MR is always 10. If
   the modifier takes the MR of a spell above 10, then the MR remains at 10 but
   the Fatigue Point cost of the spell increases by 3 FP per point above 10."
   The book's own case: "a Diviner learning a MR 8 Transmutation spell can do so
   as if it was a MR 10 spell but the casting cost would increase by 3 FP". */
const diviner = CNS5.effectiveLearningMr({
  mr: 8, method: "Transmutation Magick", mode: "Divination Mode of Magick"
});
ok("the Diviner's modifier for Transmutation", diviner.modifier, 3);
ok("which is held at ten", diviner.effective, 10);
ok("with one point over", diviner.excess, 1);
ok("costing three more Fatigue", diviner.fatigueSurcharge, 3);

/* The surcharge grows with the excess, and a spell within the bounds pays none. */
const surcharge = (mr, method, mode) =>
  CNS5.effectiveLearningMr({ mr, method, mode }).fatigueSurcharge;
ok("two points over costs six", surcharge(9, "Transmutation Magick", "Divination Mode of Magick"), 6);
ok("nothing over costs nothing", surcharge(4, "Transmutation Magick", "Divination Mode of Magick"), 0);
ok("an easier school costs nothing either", surcharge(9, "Divination Magick", "Divination Mode of Magick"), 0);

/* And the floor holds: a spell is never easier than MR 1. */
ok("never below one", CNS5.effectiveLearningMr({ mr: 1, method: "Divination Magick", mode: "Divination Mode of Magick" }).effective, 1);

/* A mage with no Mode, or a spell with no school, is taken at the table's
   figure: nothing is added or taken away. */
ok("no tradition changes nothing", CNS5.effectiveLearningMr({ mr: 6, method: "Command Magick", mode: "" }).effective, 6);
ok("no school changes nothing", CNS5.effectiveLearningMr({ mr: 6, method: "", mode: "Divination Mode of Magick" }).effective, 6);

/* The grid is held in config for a spell to use while its actor is prepared,
   and in data/magick.json as extracted. They must not drift apart. */
ok("the two copies of the grid agree",
   JSON.stringify(CNS5.learningGrid.modifiers) === JSON.stringify(magick.learningModifiers)
     && JSON.stringify(CNS5.learningGrid.modes) === JSON.stringify(magick.learningModes),
   true);
